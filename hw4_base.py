...
def expand_nr(img_mask, cur_coord):
  ...
  while len(coordinate_list) > 0:
    ...
    if img_mask[cur_coordinate[0], cur_coordinate[1]]==0 or (cur_coordinate[0]<0 or cur_coordinate[0]>img_mask.shape[0]) or (cur_coordinate[1]<0 or cur_coordinate[1]>img_mask.shape[1]):
      continue
